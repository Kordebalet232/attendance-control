        ...
        if (workers[i].exits[j] === undefined || workers[i].enters[j] === undefined) {
          ...
          if (diffDays > 1) {
            for (let k = 0; k < diffDays - 1; k++) {
              if (new Date(workers[i].exits[j].time).getDay() === 1) {
                workers[i].hours.push({ DisplayTime: "", id: id, letterDisplay: "Е" });
              } else {
                console.log(new Date(workers[i].exits[j].time));
                console.log(new Date(workers[i].exits[j].time).getDay());
                workers[i].hours.push({ DisplayTime: "", id: id, letterDisplay: "Н" });
              }
              id = id + 1;
              days = days - 1;
            }
